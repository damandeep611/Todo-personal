...
export default function Hero(){
  ...
  <defs>
    <pattern id="dense-dots" width="10" height="10" patternUnits="userSpaceOnUse">
  ...
  <rect width="100%" height="100%" fill="url(#dense-dots)" />
        ...
        <div className="relative h-32 w-32 rotate-[5deg]">
          <div className="absolute inset-0 bg-yellow-100 shadow-lg transform-gpu">
            <div className="p-4 text-sm">
              <p className="text-gray-600">Take notes to keep track of your essential tasks</p>
            </div>
      ...
      {/* element 3 */}
      <div 
          className="absolute bottom-16 right-20 hidden md:block"
        >
      ...
      <div className="container relative z-10 mx-auto px-4 py-20 pt-32 text-center">
        ...
        <motion.h1 
          ...
          >
            <AnimatePresence>
              {["Think, plan, and track", "all in one place"].map((line, lineIndex) => (
                <motion.span
                  key={line}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ duration: 0.5, delay: lineIndex * 0.5 }}
                  className={`block ${lineIndex === 1 ? "text-gray-400" : ""}`}
                >
